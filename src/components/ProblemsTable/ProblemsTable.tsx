...
const ProblemsTable: React.FC<ProblemsTableProps> = ({ setLoadingProblems }) => {
  ...
  const solvedProblems = useGetSolvedProblem();
        ...
        {problems.map((problem, idx) => {
          ...
          return (
            <tr className={`${idx % 2 == 1 ? "bg-dark-layer-1" : ""}`} key={problem.id}>
              <th className="px-2 py-4 font-medium whitespace-nowrap text-dark-green-s">{solvedProblems.includes(problem.id) && <BsCheckCircle fontSize={18} />}</th>
...
const useGetSolvedProblem = () => {
  const [solvedProblems, setSolvedProblem] = useState<string[]>([]);
