...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({ problem }) => {
  ...
  const userAndProblemData = async (transaction: any) => {
  ...
  const handleLike = async () => {
    ...
    await runTransaction(firestore, async (transaction) => {
      const { problemDoc, userDoc, userRef, problemRef } = await userAndProblemData(transaction);
      if (userDoc.exists() && problemDoc.exists()) {
        if (liked) {
          ...
          // add problem id to likedProblems on user document,remove problem id from dislikedProblems on user document,decrement dislikes on problem document, increment likes on problem document
          transaction.update(userRef, {
          ...
          //add problem id to likedProblems on user document, increment likes on problem document.
          transaction.update(userRef, {
            likedProblems: [...userDoc.data().likedProblems, problem.id],
          });
          transaction.update(problemRef, {
          ...
          <div className="w-full">
            ...
            {!loading && currentProblem && (
              <div className="flex items-center mt-3">
                ...
                <div
                  className="flex items-center cursor-pointer hover:bg-dark-fill-3 space-x-1 rounded p-[3px]
  ...
  useEffect(() => {
    const getUsersDataOnProblem = async () => {
      ...
      if (userSnap.exists()) {
        ...
        const { solvedProblems, likedProblems, dislikedProblems, starredProblem } = data;
        setData({
